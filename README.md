...
通常画面の上部で「地図」「一覧表」「比較グラフ」を切り替えられます。選択した比較物件は同じブラウザのローカル設定へ保存され、再読込後もデータセットに存在する物件だけを最大8件復元します。保存対象は公開物件IDだけで、数値・原本・社内データは保存しません。
...
「この地図範囲で検索」は、押した時点の表示範囲を既存の投資法人・用途・地域・数値条件へ追加します。「表示中から比較選択」は現在の並び順から、既存の比較対象を保持したまま最大8物件まで追加します。地図範囲外の物件を削除したり実データを書き換えたりはせず、ブラウザ内の表示だけを絞り込みます。

## 半径検索 v0.16

地図中心、地図上でクリックした任意地点、または物件詳細で選択した物件を中心として、半径1・3・5・10km以内の物件へ絞り込めます。検索範囲は青い円で表示されます。
