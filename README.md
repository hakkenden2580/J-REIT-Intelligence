...
類似物件スコアは、距離30%、賃貸可能面積20%、取得価格10%、直接還元利回り15%、最終還元利回り15%、稼働率10%の簡易ロジックです。TCR未開示時は中立ペナルティとし、値を推測しません。審査判断そのものではなく、比較候補を絞り込むために使用します。
...
比較グラフは「自動」「平均のみ」「個別＋平均」を切り替えられます。自動では8物件以下なら個別系列と平均、9物件以上なら選択物件全体の平均だけを表示し、50物件を選択しても視認性を保ちます。平均値は各開示時点で値がある物件だけを母数とし、未開示値の補間は行いません。

比較グラフと個別物件の履歴グラフは、マウスまたはポインターでなぞる・ドラッグすることで、開示時点、正確な値、平均の算定対象物件数を確認できます。この操作は表示上の照会だけであり、公開値やEvidenceを書き換えません。

物件詳細では、最新指標に加え、投資法人、証券コード、用途、地域、賃貸可能面積、賃貸面積、テナント数、割引率、最終還元利回り、座標精度を整理して表示します。取得日、竣工年、最寄駅、持分形態など未収集の項目は推測表示せず、Data EngineとEvidence契約を拡張した後に追加します。

## 安定チャートと欠損区間表示 v0.19

比較グラフと個別物件履歴は、データ線を描く固定キャンバスと、カーソル・ツールチップだけを描く操作レイヤーへ分離しました。カーソル移動時にキャンバス本体の寸法を再計算しないため、グラフへカーソルを合わせた際の無限拡大を防ぎます。画面幅の変化は`ResizeObserver`で検知し、実際に表示領域が変わった場合だけ再描画します。

未開示時点をゼロや推定値で埋めず、開示値の実線と未開示期間をまたぐ点線を分けて表示します。点線は前後の開示値を視覚的に関連付けるだけで、その期間の値を補間したものではありません。平均線を強調し、横罫線・軸ラベル・凡例・黒背景ツールチップを整理して、多数物件でも読み取りやすい分析カードにしました。

## 半期整列と母数基準付き平均 v0.20

NBF・JRE・GLPは決算月が異なるため、従来の比較グラフでは同じ市況を示す開示値が別々の月に配置され、時点によって平均母数が1件まで減ることがありました。v0.20では各開示日を暦年の上期（1〜6月）・下期（7〜12月）へ正規化し、同じ半期の開示値を共通の比較時点へ揃えます。同一物件に同一半期の開示が複数ある場合は、その半期で最も新しい公開値を使用します。

9物件以上を選択した場合、平均線は選択数の10%かつ最低3件以上の開示値がある半期だけを表示します。母数不足の半期は推定せず、ツールチップには「開示件数 / 選択件数」とカバレッジ率を表示します。

## 分布を含む比較分析 v0.21

多数物件の市場推移を平均値だけで判断しないよう、比較グラフへ中央値と第1四分位〜第3四分位の帯（中央50%）を追加しました。中央値は外れ値の影響を受けにくく、四分位帯は選択物件群のばらつきを示します。最新の比較時点、平均値、中央値、中央50%、開示カバレッジはグラフ上部のサマリーカードで確認できます。

各半期のツールチップでも平均・中央値・中央50%・開示母数を同時に表示します。分布指標にもv0.20と同じ最低母数基準を適用し、母数不足の期間を推定値で補いません。

## 最終還元利回りと鑑定Evidence v0.22

- 最終還元利回り（TCR／ターミナルキャップレート）を直接還元利回り（CR）と区別した正式指標として追加
- 物件詳細でCR、TCR、`TCR − CR`スプレッド、割引率を一画面に表示
- TCRを物件別・複数物件の時系列グラフで比較
- 地図の色・数値表示を直接還元CR／最終還元TCRで切替
- 各時点のプロットへマウスを合わせると、値とExcelシート・セルまたはPDFページを表示
- 最新指標カードからEvidenceの取得日時、抽出方法、確認状態を照合
- 一覧表、CSV、条件検索へTCRを追加
- 類似物件スコアへTCRを15%組み込み、未開示値は推測せず中立ペナルティとして扱う
- 数値とEvidenceの指標コード・単位・値が一致しない取込を品質Gateで拒否

NBFは物件別Excelに開示された複数期のTCR、GLPは利用可能な最新期のTCRを取り込みます。JREなど原資料にTCRがない法人・期は空欄のまま保持します。CRとTCRは意味の異なる指標であり、相互代用や自動補完は行いません。
